...
export const getKitchenLatestRecipes = async (limit = 20) => {
  return await useApiJson<ApiResponseWithData<{items: KitchenRecipe[]; total_public?: number}>>(
    `/kitchen/recipes/latest?limit=${limit}`,
    {
      method: "GET"
    }
  )
...
export const favoriteKitchenIngredient = async (payload: {
...
export const unfavoriteKitchenIngredient = async (
  ingredientId: number,
  ...
  return await useApiJson<ApiResponseWithData<{deleted: boolean}>>(
    `/kitchen/ingredients/favorites/${ingredientId}${suffix ? `?${suffix}` : ""}`,
    {
      method: "DELETE"
    }
  )
