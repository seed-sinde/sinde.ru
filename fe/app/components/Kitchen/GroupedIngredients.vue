      ...
      <section v-for="group in groupedItems" :key="`group:${group.category}`" class="min-w-0">
        ...
        <ul class="space-y-1.5">
          <li
            ...
            <span class="min-w-0 truncate tabular-nums text-zinc-200">{{ amountUnitText(entry.item) }}</span>
            <span class="min-w-0 truncate text-zinc-400 leading-tight">{{ noteText(entry.item) }}</span>
          </li>
        </ul>
      </section>
    </div>
