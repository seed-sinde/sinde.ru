  ...
  <div class="space-y-4">
    ...
    <label class="block text-xs text-zinc-400">
      ...
      <LabBaseTextarea
        ...
        class="mt-1 w-full"></LabBaseTextarea>
    </label>
    <LabSpoiler
      title="Детали"
      show-label="Показать"
      ...
      toggle-button-class="inline-flex h-8 items-center gap-1.5 rounded-md border border-zinc-700 bg-zinc-900 px-2.5 text-xs text-zinc-200 transition hover:bg-zinc-800"
      ...
      <div class="flex flex-wrap items-end gap-2">
        <label class="w-full text-xs text-zinc-400 sm:w-[calc(50%-0.25rem)] xl:w-52">
          ...
          <datalist id="kitchen-meal-type-list">
            <option v-for="item in mealTypeSuggestions" :key="`meal:${item}`" :value="item"></option>
          </datalist>
        </label>
        <label class="w-full text-xs text-zinc-400 sm:w-[calc(50%-0.25rem)] xl:w-52">
          Тип питания
          ...
          <datalist id="kitchen-diet-type-list">
            <option v-for="item in dietTypeSuggestions" :key="`diet:${item}`" :value="item"></option>
          </datalist>
        </label>
        <label class="w-full text-xs text-zinc-400 sm:w-[calc(50%-0.25rem)] xl:w-52">
          ...
          <datalist id="kitchen-cooking-method-list">
            <option v-for="item in cookingMethodSuggestions" :key="`cook:${item}`" :value="item"></option>
          </datalist>
        </label>
        <label class="w-full text-xs text-zinc-400 sm:w-[calc(50%-0.25rem)] xl:w-52">
          Подготовка (мин)
        ...
        <label class="w-full text-xs text-zinc-400 sm:w-[calc(50%-0.25rem)] xl:w-52">
          ...
          <datalist id="kitchen-nationality-list">
            <option v-for="item in nationalitySuggestions" :key="`nationality:${item}`" :value="item"></option>
          </datalist>
        </label>
        <LabBaseScale
    ...
    <div class="space-y-2">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <LabBaseButton v-if="coverImageDraft.image_key" @click="clearCoverImage" size="sm" variant="danger">
          Убрать фото
        </LabBaseButton>
      </div>
    ...
    <div class="grid gap-2 md:grid-cols-6">
      ...
      <datalist id="kitchen-unit-list">
        <option v-for="item in unitSuggestions" :key="`unit:${item}`" :value="item"></option>
      </datalist>
    </div>
    <div class="space-y-2">
        ...
        <li
          ...
          <div class="flex flex-col gap-3 md:flex-row md:items-start">
            <div class="flex items-center gap-2 md:w-28 md:shrink-0">
              ...
              <div class="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.06em] lab-text-soft">
                <span
                  class="border-[color-mix(in_srgb,var(--lab-border)_72%,transparent)] inline-flex h-5 w-5 items-center justify-center rounded-full border bg-transparent text-[10px] font-semibold lab-text-primary">
                  {{ idx + 1 }}
                </span>
                <span>шаг</span>
              </div>
      ...
      <Teleport to="body">
        <div
          ...
          <div class="overflow-hidden rounded-xl border border-cyan-400/60 bg-zinc-950/95 p-3 backdrop-blur">
            <div class="flex items-center gap-2 text-[10px] uppercase tracking-[0.08em]">
            ...
            <p class="mt-2 whitespace-pre-wrap text-sm leading-5 text-zinc-100">
