  ...
  <div class="max-w-sm space-y-2">
    ...
    <div class="relative" @click="focus">
      ...
      <div :class="segmentsWrapClass">
        ...
        <span class="text-lg font-semibold text-(--lab-text-muted)">-</span>
        <div :class="segmentClass(rightValue, 'right')">
          {{ rightValue }}
        </div>
      </div>
    </div>
    <p v-if="resolvedHint" class="text-xs" :class="rejectedHint ? 'text-(--lab-danger)' : 'text-(--lab-text-muted)'">
      {{ resolvedHint }}
    </p>
  </div>
</template>
...
const segmentsWrapClass = computed(() => [
  ...
  props.invalid ? 'ring-(--lab-danger) ring-1' : props.valid ? 'ring-(--lab-success) ring-1' : ''
])
