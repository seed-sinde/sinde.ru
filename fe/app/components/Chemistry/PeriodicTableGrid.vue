...
const elementCardStyle = (element: PeriodicTableElement) => {
  ...
  if (dimmedSet.value.has(elementNumber)) {
    if (effectiveTheme.value === 'light') {
      return {
        borderColor: 'color-mix(in srgb, var(--lab-border) 90%, transparent)',
        background: 'color-mix(in srgb, var(--lab-bg-surface-muted) 94%, rgb(228 228 231))'
      }
    }
    return {
      borderColor: 'color-mix(in srgb, var(--lab-border) 88%, transparent)',
      background: 'color-mix(in srgb, var(--lab-bg-surface) 94%, rgb(24 24 27))'
    }
  }
  if (props.useThemeCardColors) {
    if (highlightedSet.value.has(elementNumber)) {
      if (effectiveTheme.value === 'light') {
        return {
          borderColor: `color-mix(in srgb, ${categoryColor} 88%, var(--lab-border))`,
          background: `color-mix(in srgb, ${categoryColor} 92%, white)`
        }
      }
      return {
        borderColor: `color-mix(in srgb, ${categoryColor} 64%, var(--lab-border))`,
        background: `color-mix(in srgb, ${categoryColor} 30%, var(--lab-bg-surface))`
      }
    }
    if (effectiveTheme.value === 'light') {
      return {
...
<template>
  <div class="flex min-h-0 w-full max-w-full items-start justify-start overflow-x-auto overflow-y-hidden">
    <div :class="frameClass" :style="frameStyle">
      <div
        class="grid h-full w-full grid-cols-1 grid-rows-1 gap-(--periodic-gap) lg:grid-cols-[var(--periodic-row-label-size)_minmax(0,1fr)] lg:grid-rows-[var(--periodic-column-label-size)_minmax(0,1fr)]"
      >
        <div class="hidden lg:block" aria-hidden="true" />
        <div class="hidden gap-(--periodic-gap) text-center lg:grid" :style="tableColumnHeaderStyle">
          <div
            v-for="group in groupHeaders"
            :key="`group-header:${group}`"
            class="flex items-center justify-center text-[0.55rem] font-medium tracking-[0.12em] text-(--lab-text-muted) md:text-[0.68rem]"
            :class="compact ? 'lg:text-[0.5rem]' : ''"
          >
            {{ group }}
          </div>
        </div>
        <div class="hidden h-full min-h-0 gap-(--periodic-gap) text-right lg:grid" :style="tableRowHeaderStyle">
          <div
            v-for="(period, index) in periodHeaders"
            :key="`period-header:${visiblePeriodRows[index]}`"
            class="flex items-center justify-end pr-1 text-[0.55rem] font-medium tracking-[0.12em] text-(--lab-text-muted) md:text-[0.68rem]"
            :class="compact ? 'lg:text-[0.5rem]' : ''"
            aria-hidden="true"
          >
            {{ period }}
        ...
        <div class="relative grid h-full min-h-0 min-w-0 gap-(--periodic-gap)" :style="tableGridStyle">
          ...
          <button
            ...
            class="relative z-10 flex h-full min-h-0 w-full min-w-0 flex-col overflow-hidden border p-0.5 px-1 py-1 text-left sm:px-2 sm:py-1.5 lg:p-1"
            :aria-disabled="isElementDisabled(element.number) ? 'true' : undefined"
            :class="[
              elementUnavailableClass(element.number),
              dimmedSet.has(element.number) ? 'text-(--lab-text-muted) opacity-55 grayscale' : '',
              isElementDisabled(element.number)
                ? 'cursor-not-allowed opacity-100 ring-1 saturate-[0.8] ring-inset'
                : 'opacity-100',
          ...
          >
            <span
              v-if="compact"
              class="flex flex-1 items-center justify-center text-[1.05rem] leading-none font-semibold lg:text-[0.82rem]"
            ...
            <span
              v-else
              class="flex flex-1 items-center justify-center text-[1.05rem] leading-none font-semibold 2xl:hidden"
              :class="isElementDisabled(element.number) ? 'text-(--lab-text-muted)' : 'text-(--lab-text-primary)'"
            >
              {{ element.displaySymbol }}
            </span>
            <div v-if="!compact" class="hidden h-full min-w-0 flex-col 2xl:flex">
              <div class="flex items-start justify-between gap-2">
                ...
                <span class="pt-0.5 text-[0.68rem] leading-none font-medium text-(--lab-text-muted)">
                  {{ element.number }}
                </span>
              </div>
              <div class="mt-1 min-w-0">
                <span
                  class="block overflow-hidden leading-[1.2] font-medium text-ellipsis whitespace-nowrap"
                ...
                <span
                  class="mt-0.5 block overflow-hidden leading-[1.2] text-ellipsis whitespace-nowrap text-(--lab-text-muted)"
