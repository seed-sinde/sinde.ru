<script setup lang="ts">
  ...
  const getLocaleMessageKey = (code: InterfaceLocaleCode): InterfaceMessageKey =>
    `locale.${code}` as InterfaceMessageKey
    ...
    <template #trigger="{ toggle }">
      <div class="relative inline-flex">
        <LabBaseButton
          ...
          @click="toggle" />
        <span
          class="pointer-events-none absolute -right-0.5 top-1.5 inline-flex h-2.8 min-w-1 items-center justify-center border border-zinc-600 bg-zinc-900 px-0.5 text-[8px] leading-none text-zinc-200">
          {{ selectedLocaleBadge }}
        </span>
      </div>
    </template>
