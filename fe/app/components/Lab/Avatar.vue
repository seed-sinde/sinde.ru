...
const hasAdminNotifications = computed(() => {
  ...
  if (typeof summary.has_unread === 'boolean') return summary.has_unread
  return (
    Number(summary.new_users_since_last_login || 0) > 0 || Number(summary.new_pending_recipes_since_last_login || 0) > 0
  )
})
...
<template>
  <NuxtLink
    ...
    :to="authStatusTo"
    class="relative inline-flex min-w-0 items-center gap-1.5 text-(--lab-text-muted) transition-colors hover:text-(--lab-text-primary)"
