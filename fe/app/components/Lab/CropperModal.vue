...
const localAspectPreset = ref<CropperAspectPreset>('free')
...
const currentAspectRatio = computed(() => {
  if (localAspectPreset.value === '1:1') return 1
  if (localAspectPreset.value === '4:3') return 4 / 3
  if (localAspectPreset.value === '16:9') return 16 / 9
  if (localAspectPreset.value === 'custom') return parsedCustomAspect.value
  return null
})
...
const applyAspectRatio = () => {
  ...
  if (localAspectPreset.value === 'custom') {
  ...
  cropper = new Cropper(imageRef.value, {
    ...
    ready: () => {
      ...
      if (naturalWidth > 0 && naturalHeight > 0) {
        ...
        if (localAspectPreset.value === 'free') {
...
const invertAspectRatio = () => {
  if (localAspectPreset.value === 'free') return
  if (localAspectPreset.value === '1:1') return
  if (localAspectPreset.value === '4:3') {
    customAspectWidth.value = '3'
    customAspectHeight.value = '4'
    localAspectPreset.value = 'custom'
    return
  }
  if (localAspectPreset.value === '16:9') {
...
const applyCrop = async () => {
  ...
  const ratio = currentAspectRatio.value
  if (localAspectPreset.value === 'custom' && (!ratio || !Number.isFinite(ratio) || ratio <= 0)) {
...
watch(
  ...
  async file => {
    ...
    zoomRatio.value = 1
    originalImageAspect.value = null
    localAspectPreset.value = props.aspectPreset
...
watch([localAspectPreset, customAspectWidth, customAspectHeight], () => {
  applyAspectRatio()
})
watch(
  () => props.aspectPreset,
...
<template>
  <div class="fixed inset-0 z-60 bg-(--lab-bg-canvas)" @click.self="emit('cancel')">
    <div class="flex h-full w-full flex-col bg-(--lab-bg-canvas)">
      <div class="z-10" @click.stop>
        <div class="relative">
          <div
            ref="headerScrollerRef"
            class="lab-scroll-hidden w-full overflow-x-auto overflow-y-hidden border-b bg-(--lab-bg-canvas) px-2.5 py-1.5 text-(--lab-text-secondary) sm:px-3"
          >
      ...
      <div
        ...
        class="relative min-h-0 flex-1 overflow-hidden bg-(--lab-bg-canvas) px-2 py-2 sm:px-3 sm:py-3"
      >
        <div
          class="h-full w-full overflow-hidden border bg-(--lab-bg-surface-muted) [&_.cropper-container]:h-full [&_.cropper-container]:w-full [&_.cropper-dashed]:border-white/20 [&_.cropper-face]:bg-transparent [&_.cropper-line]:bg-white/85 [&_.cropper-point]:bg-white/85 [&_.cropper-view-box]:outline [&_.cropper-view-box]:outline-white/40"
          @click.stop
        >
          <img
            ref="imageRef"
          ...
          <div ref="controlsScrollerRef" class="lab-scroll-hidden overflow-x-auto overflow-y-hidden">
            <div class="flex min-w-max items-center gap-3">
              <div class="flex shrink-0 items-center gap-2">
                ...
                <div
                  v-if="!props.aspectLocked && localAspectPreset === 'custom'"
