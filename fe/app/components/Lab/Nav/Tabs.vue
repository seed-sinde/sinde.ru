...
<script setup lang="ts">
  ...
  const resolvedButtonClass = computed(() =>
    twMerge(
      'text-(--lab-text-muted) focus-visible:text-(--lab-text-primary) relative inline-flex h-9 shrink-0 items-center justify-center border-0 border-b-2 border-transparent bg-transparent px-2.5 text-xs font-medium whitespace-nowrap outline-none transition-colors',
      props.buttonClass
    )
  )
