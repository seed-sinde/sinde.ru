<template>
  <div class="space-y-3 bg-transparent p-0">
    ...
    <div v-else-if="mode === 'lab'" class="space-y-2">
      <p class="text-xs font-medium uppercase tracking-[0.08em] text-zinc-500">Слайдеры LAB</p>
      <div class="space-y-1">
      ...
      <p class="pt-1 text-xs font-medium uppercase tracking-[0.08em] text-zinc-500">Ручной ввод LAB</p>
      ...
      <div class="flex items-center gap-3">
        <div class="h-10 w-14 rounded-xl border border-zinc-700/80" :style="{ background: labPreviewColor }"></div>
    ...
    <div v-else class="space-y-2">
      <LabField label="Категория цвета" for-id="color-spectrum">
        <LabBaseSelect
          ...
          name="color_spectrum"
          v-model="spectrum"
          :options="spectrumSelectOptions" />
      </LabField>
      <div class="h-10 w-14 rounded-xl border border-zinc-700/80" :style="{ background: spectrumColor }"></div>
    </div>
  </div>
</template>
<script setup lang="ts">
  ...
  const lockedMode = computed<Color['mode'] | ''>(() => {
    return resolveColorMode(
      props.meta && typeof props.meta === 'object' ? (props.meta as Record<string, unknown>) : null
    )
  })
  watchEffect(() => {
