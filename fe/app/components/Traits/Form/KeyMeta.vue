<template>
  <div class="grid gap-3">
    <LabBaseField label="Тип данных" for-id="trait-meta-type">
    ...
    <div v-if="form.dataType === 'string'" class="grid gap-2 md:grid-cols-2">
      <LabBaseField label="Минимальная длина" for-id="trait-meta-min-length">
        <LabBaseInput id="trait-meta-min-length" v-model="minLengthText" type="number" inputmode="numeric" />
      </LabBaseField>
      <LabBaseField label="Максимальная длина" for-id="trait-meta-max-length">
    ...
    <div v-else-if="form.dataType === 'boolean'" class="grid gap-2 md:grid-cols-2">
      <LabBaseField label="Способ отображения" for-id="trait-meta-boolean-display">
        <LabBaseSelect id="trait-meta-boolean-display" v-model="form.booleanDisplay" :options="booleanDisplayOptions" />
      </LabBaseField>
      <div class="grid gap-2">
        <LabBaseField label="Подпись true" for-id="trait-meta-boolean-true">
          <LabBaseInput id="trait-meta-boolean-true" v-model="form.booleanTrueLabel" />
        </LabBaseField>
        <LabBaseField label="Подпись false" for-id="trait-meta-boolean-false">
    ...
    <div v-else-if="form.dataType === 'list'" class="grid gap-2">
      ...
      <LabBaseField label="Минимум значений" for-id="trait-meta-list-min-items">
      ...
      <LabBaseField
        ...
        for-id="trait-meta-options"
        hint="Необязательно. По одному значению на строку."
      >
    ...
    <div v-else-if="form.dataType === 'range'" class="grid gap-2 md:grid-cols-2">
      <LabBaseField label="Тип диапазона" for-id="trait-meta-range-type">
        <LabBaseSelect id="trait-meta-range-type" v-model="form.rangeType" :options="rangeTypeOptions" />
      </LabBaseField>
      <LabBaseField label="Часовой пояс" for-id="trait-meta-range-timezone">
        <LabBaseInput id="trait-meta-range-timezone" v-model="form.timezone" placeholder="UTC" />
      </LabBaseField>
    </div>

    <div
      v-else-if="form.dataType === 'datetime' || form.dataType === 'interval' || form.dataType === 'schedule' || form.dataType === 'validity'"
      class="grid gap-2 md:grid-cols-2"
    >
      <LabBaseField label="Часовой пояс" for-id="trait-meta-timezone">
        <LabBaseInput id="trait-meta-timezone" v-model="form.timezone" placeholder="UTC" />
      </LabBaseField>
      <LabBaseField v-if="form.dataType === 'interval'" label="Единица длительности" for-id="trait-meta-duration-unit">
        <LabBaseInput id="trait-meta-duration-unit" v-model="form.durationUnit" placeholder="minutes" />
      </LabBaseField>
      <LabBaseField v-if="form.dataType === 'validity'" label="Условное событие" for-id="trait-meta-event">
    ...
    <div v-else-if="form.dataType === 'geo'" class="grid gap-2 md:grid-cols-2">
      <LabBaseField label="Подтип" for-id="trait-meta-geo-type">
        <LabBaseSelect id="trait-meta-geo-type" v-model="form.geoType" :options="geoTypeOptions" />
      </LabBaseField>
      <LabBaseField v-if="form.geoType === 'point'" label="Единица высоты" for-id="trait-meta-height-unit">
        <LabBaseInput id="trait-meta-height-unit" v-model="form.heightUnit" placeholder="m" />
      </LabBaseField>
      <LabBaseField v-if="form.geoType === 'zone'" label="Единица радиуса" for-id="trait-meta-radius-unit">
    ...
    <div v-else-if="form.dataType === 'color'" class="grid gap-2 md:grid-cols-2">
      <LabBaseField label="Режим" for-id="trait-meta-color-mode">
        <LabBaseSelect id="trait-meta-color-mode" v-model="form.mode" :options="colorModeOptions" />
      </LabBaseField>
      <LabBaseField label="Область применения" for-id="trait-meta-color-domain">
        <LabBaseInput id="trait-meta-color-domain" v-model="form.colorDomain" placeholder="дизайн, медицина" />
      </LabBaseField>
      <LabBaseField label="Стандарт" for-id="trait-meta-color-standard">
        <LabBaseInput
          id="trait-meta-color-standard"
          v-model="form.colorStandard"
          placeholder="RAL, Fitzpatrick, VITA"
        />
      </LabBaseField>
      <LabBaseField label="Условия измерения" for-id="trait-meta-color-conditions">
        <LabBaseInput
          id="trait-meta-color-conditions"
          v-model="form.measurementConditions"
          placeholder="свет, угол, наблюдение"
        />
      </LabBaseField>
      <LabBaseField label="Фактура поверхности" for-id="trait-meta-color-texture">
        <LabBaseInput id="trait-meta-color-texture" v-model="form.surfaceTexture" />
      </LabBaseField>
      <LabBaseField label="Прозрачность 0..1" for-id="trait-meta-color-opacity">
        <LabBaseInput
          id="trait-meta-color-opacity"
          v-model="opacityText"
          type="number"
          inputmode="decimal"
          step="0.01"
      ...
      <LabBaseField label="Именованный цвет" for-id="trait-meta-color-name">
        <LabBaseInput id="trait-meta-color-name" v-model="form.namedColor" />
      </LabBaseField>
      <LabBaseField label="Палитра" for-id="trait-meta-color-palette">
    ...
    <div v-else-if="form.dataType === 'surface'" class="grid gap-2 md:grid-cols-2">
      <LabBaseField label="Категория блеска" for-id="trait-meta-gloss">
        <LabBaseInput id="trait-meta-gloss" v-model="form.glossCategory" placeholder="матовый, глянец" />
      </LabBaseField>
      <LabBaseField label="Тип рельефа" for-id="trait-meta-relief">
        <LabBaseInput id="trait-meta-relief" v-model="form.reliefType" placeholder="песок, кожа, сатин" />
      </LabBaseField>
    </div>

    <LabBaseField label="Проверка / валидация" for-id="trait-meta-validate">
