<template>
  <div
    ...
    tabindex="0"
    class="lab-focus group cursor-pointer border p-3 transition select-none"
    :class="
      selected
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="min-w-0">
            <div class="flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1">
              ...
              <span v-if="numberUnit" class="lab-text-muted shrink-0 font-mono text-xs">
                {{ numberUnit }}
              </span>
            </div>
          ...
          <NuxtLink
            :to="detailLinkTo"
            class="lab-focus shrink-0 text-xs tracking-[0.14em] text-(--lab-text-muted) uppercase transition hover:text-(--lab-accent) focus-visible:text-(--lab-accent)"
