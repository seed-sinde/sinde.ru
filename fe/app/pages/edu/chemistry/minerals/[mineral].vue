    ...
    <section class="p-4">
      ...
      <div v-else-if="mineral">
        <div :class="hasMineralImages ? 'grid gap-2 xl:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]' : 'space-y-2'">
          <div class="space-y-2">
            <div class="space-y-2">
              <div class="space-y-2">
                ...
                <div v-if="mineralSubtitle" class="text-sm wrap-break-word text-zinc-500">{{ mineralSubtitle }}</div>
              </div>
            ...
            <div v-if="hasMineralImages" class="space-y-3">
              <div class="text-xs tracking-[0.08em] text-zinc-500 uppercase">Изображения</div>
          ...
          <div class="space-y-3">
            <div v-for="item in chemistryDisplayItems" :key="`chemistry:${item.label}`" class="min-w-0">
              <div class="text-[10px] tracking-[0.08em] text-zinc-500 uppercase">{{ item.label }}</div>
              <LabViewerLaTex v-if="item.isFormula" :formula="item.latex" />
              <div v-else class="mt-1 text-base leading-6 wrap-break-word text-zinc-100">{{ item.value }}</div>
            </div>
            <div v-if="chemistryElementChips.length" class="min-w-0">
              <div class="text-[10px] tracking-[0.08em] text-zinc-500 uppercase">Chemistry elements</div>
            ...
            <div v-if="valenceElementChips.length" class="min-w-0">
              <div class="text-[10px] tracking-[0.08em] text-zinc-500 uppercase">Valence elements</div>
              <div class="mt-2 flex flex-wrap gap-2">
            ...
            <div class="text-xs tracking-[0.08em] text-zinc-500 uppercase">Классификация</div>
            <div class="grid grid-cols-1 gap-3 min-[720px]:grid-cols-2 min-[1400px]:grid-cols-4">
              <div
              ...
              >
                <div class="text-[10px] tracking-[0.08em] text-zinc-500 uppercase">{{ item.label }}</div>
                <div class="mt-1 text-sm leading-5 wrap-break-word text-zinc-100">{{ item.value }}</div>
              </div>
            </div>
            <div
            ...
            <div v-if="notesItems.length" class="space-y-3">
              <div class="text-xs tracking-[0.08em] text-zinc-500 uppercase">Status notes</div>
              <div v-for="item in notesItems" :key="`notes:${item.label}`" class="min-w-0">
                <div class="mt-1 text-sm leading-6 wrap-break-word text-zinc-100">{{ item.value }}</div>
              </div>
            </div>
            <div v-for="item in originItems" :key="`origin:${item.label}`" class="min-w-0">
              <div class="text-[10px] tracking-[0.08em] text-zinc-500 uppercase">{{ item.label }}</div>
              <div class="mt-1 text-sm leading-5 wrap-break-word text-zinc-100">{{ item.value }}</div>
            </div>
            <div v-if="parageneticModeItems.length" class="min-w-0">
              <div class="text-[10px] tracking-[0.08em] text-zinc-500 uppercase">Paragenetic modes</div>
              <div class="mt-2 flex flex-wrap gap-2">
                <span
