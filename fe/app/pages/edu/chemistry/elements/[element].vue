<script setup lang="ts">
  ...
  const formatListText = (values: Array<string | number | null | undefined>, fallback = NO_DATA) => {
    const items = values.map(trimText).filter(Boolean)
...
<template>
  <div>
    ...
    <section class="p-4 grid gap-0 xl:grid-cols-[minmax(0,1fr)_minmax(18rem,24rem)]">
      <div class="min-w-0">
        <div class="flex items-start gap-4">
          <div
            class="grid h-18 w-18 shrink-0 place-items-center border text-4xl font-semibold sm:h-20 sm:w-20 sm:text-5xl"
        ...
        <dl class="mt-4 flex flex-wrap gap-2">
          <div
            ...
            :key="stat.label"
            class="min-w-32 max-w-full bg-(--lab-bg-surface-subtle) px-3 py-2">
            <dt class="text-[11px] uppercase tracking-[0.07em] text-(--lab-text-muted)">
              {{ stat.label }}
            </dt>
            <dd class="mt-1 text-sm font-medium leading-5">
              {{ stat.value }}
            </dd>
          </div>
        </dl>
        ...
        <article class="min-w-0 px-4 pb-4 sm:px-6 sm:pb-6">
          <div class="grid gap-x-6 gap-y-6 md:grid-cols-2 2xl:grid-cols-3">
            <section v-for="section in detailSections" :key="section.title" class="min-w-0">
              <h2 class="mb-3 text-sm font-semibold uppercase tracking-[0.07em] text-(--lab-text-muted)">
                {{ section.title }}
              </h2>
          ...
          <article class="min-w-0">
            <section class="space-y-4">
              ...
              <section v-else-if="currentElement.sampleFallback" class="lab-surface-warning p-4">
                <div class="text-xs uppercase tracking-[0.07em]">
