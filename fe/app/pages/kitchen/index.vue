...
const upsertFavoriteIngredient = (item: KitchenFavoriteIngredient) => {
  ...
  favoriteIncludeIngredients.value = upsertKitchenFavoriteIngredient(favoriteIncludeIngredients.value, item, 'include')
}
...
const showRecipeManageSection = computed(
  () =>
    activeKitchenTab.value === 'my-recipes' || activeKitchenTab.value === 'create' || activeKitchenTab.value === 'edit'
)
...
const clearRecipeFavoriteState = () => {
  for (const key of Object.keys(recipeFavoriteMap)) {
    recipeFavoriteMap[key] = false
  }
  for (const key of Object.keys(recipeFavoritePendingMap)) {
    recipeFavoritePendingMap[key] = false
  }
}
...
const toggleRecipeFavorite = async (recipe: KitchenRecipe) => {
  ...
  try {
    ...
    recipeFavoritePendingMap[id] = false
  }
}
...
const selectedIngredientChipItems = computed(() => toKitchenTagChipItems(displaySelectedIngredients.value, 'selected:'))
const excludedIngredientChipItems = computed(() => toKitchenTagChipItems(displayExcludedIngredients.value, 'excluded:'))
...
const catalogCardMetaClass = computed(() =>
  kitchenTheme.value === 'light' ? 'text-[10px] leading-tight text-zinc-600' : 'text-[10px] leading-tight text-zinc-500'
)
...
const removeCustomIngredient = async (item: KitchenUserIngredient) => {
  ...
  try {
    ...
    accountIngredientsError.value = err?.data?.message || err?.message || 'Не удалось удалить персональный ингредиент.'
  } finally {
...
const removeFavoriteIngredientChip = async (item: KitchenFavoriteIngredient, listType: 'include' | 'exclude') => {
  ...
  try {
    ...
    accountIngredientsError.value = err?.data?.message || err?.message || 'Не удалось удалить ингредиент из избранного.'
  } finally {
    ...
    <section v-if="activeKitchenTab === 'ingredients'" class="space-y-2 p-4">
      ...
      <div v-else-if="isAuthenticated" class="space-y-2 p-2">
        ...
        <div class="grid gap-3 md:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
          <div class="space-y-2">
            <p class="text-xs tracking-[0.06em] text-zinc-500 uppercase">Добавить свой ингредиент</p>
          ...
          <div class="space-y-3">
            <div class="space-y-2">
              <p class="text-xs tracking-[0.06em] text-zinc-500 uppercase">Любимые ингредиенты (для включения)</p>
              <KitchenRemovableChipList
            ...
            <div class="space-y-2">
              <p class="text-xs tracking-[0.06em] text-zinc-500 uppercase">Исключаемые ингредиенты</p>
              <KitchenRemovableChipList
