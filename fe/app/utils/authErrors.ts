...
const isCsrfErrorMessage = (message: string) => {
  ...
  return normalized.includes('csrf')
    || normalized.includes('проверка csrf')
    || normalized.includes('сессия защиты')
}
const isInvalidTwoFactorCodeMessage = (message: string) => {
  const normalized = normalizeErrorMessage(message)
  return normalized.includes('invalid 2fa code')
...
export const isMfaTicketExpiredMessage = (message: string) => {
  const normalized = normalizeErrorMessage(message)
  return normalized === 'mfa ticket expired'
...
export const getTwoFactorErrorMessage = (
  err: unknown,
  kind: 'totp' | 'backup',
  fallback: string,
) => {
