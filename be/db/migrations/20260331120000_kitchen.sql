...
CREATE INDEX IF NOT EXISTS idx_kitchen_recipes_created_at ON kitchen_recipes(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_kitchen_recipes_title_lower ON kitchen_recipes(lower(title));
CREATE INDEX IF NOT EXISTS idx_kitchen_recipes_ingredients_search_gin ON kitchen_recipes USING GIN(ingredients_search);
CREATE INDEX IF NOT EXISTS idx_kitchen_recipes_diet_type ON kitchen_recipes(diet_type);
CREATE INDEX IF NOT EXISTS idx_kitchen_recipes_cooking_method ON kitchen_recipes(cooking_method);
CREATE INDEX IF NOT EXISTS idx_kitchen_recipes_owner_user_id ON kitchen_recipes(owner_user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_kitchen_recipes_moderation_status_created ON kitchen_recipes(moderation_status, created_at DESC);
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION set_kitchen_recipes_updated_at()
  RETURNS TRIGGER
  AS $fn$
BEGIN
  NEW.updated_at = now();
...
CREATE INDEX IF NOT EXISTS idx_kitchen_user_ingredients_user_id ON kitchen_user_ingredients(user_id, created_at DESC);
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION set_kitchen_user_ingredients_updated_at()
  RETURNS TRIGGER
  AS $fn$
BEGIN
  NEW.updated_at = now();
...
CREATE INDEX IF NOT EXISTS idx_kitchen_ingredients_catalog_category_id ON kitchen_ingredients_catalog(category_id, sort_order, ingredient_id);
CREATE INDEX IF NOT EXISTS idx_kitchen_filter_options_kind ON kitchen_filter_options(kind, sort_order, option_id);
...
CREATE INDEX IF NOT EXISTS idx_kitchen_favorite_ingredients_user_id ON kitchen_favorite_ingredients(user_id, created_at DESC);
CREATE UNIQUE INDEX IF NOT EXISTS uq_kitchen_favorite_ingredients_user_normalized ON kitchen_favorite_ingredients(user_id, normalized_name, list_type);
...
CREATE INDEX IF NOT EXISTS idx_kitchen_favorite_recipes_user_id ON kitchen_favorite_recipes(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_kitchen_favorite_recipes_recipe_id ON kitchen_favorite_recipes(recipe_id, created_at DESC);
-- +goose Down
