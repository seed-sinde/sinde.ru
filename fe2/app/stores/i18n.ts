...
export const useI18nStore = defineStore("i18n", () => {
  ...
  const sectionKey = (locale: InterfaceLocaleCode, namespace: string) => `${locale}:${namespace}`
  const getSection = (locale: InterfaceLocaleCode, namespace: string) =>
    sections.value[sectionKey(locale, namespace)] || null

  const loadSection = async (locale: InterfaceLocaleCode, namespace: string) => {
    ...
    const request = api
      .json<{data?: I18nMessages}>(`/i18n/${locale}/${namespace}`)
      .then(res => {
        const data = res.data || {}
  ...
  const t = (locale: InterfaceLocaleCode, namespace: string, key: string) =>
